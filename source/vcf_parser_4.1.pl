...
while (<$filehandle>)
{
  ...
  else
  {
    ...
    my $id     = $line_array[2];
    my $ref    = $line_array[3];
    my $alt_field = $line_array[4];
    my $alt_state = get_state($alt_field);
    ...
    # The alt_state flag is 1 if the length of the SNP is > 1
    if ( length($ref) > 1 || $alt_state == 1)
    {
      $dataflag = 0;
    }
...
sub get_state
{
  my $state = 0;
  my $alt_string = $_[0];
  my @alt_array = split(",", $alt_string);

  my $last_element = pop @alt_array;
  if ( $last_element eq "<NON_REF>")
  {
    for( @alt_array)
    {
      if (length($_) > 1)
