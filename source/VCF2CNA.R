...
if (!setMap) {
	...
	if (hg18) {
        ...
        if (GRCh38) map.prefix = (paste(base_dir, "/mapability/hg38/", sep=""))
    }
}
...
cn.D = cn.D[-rm.idx,]
	...
	if (singleBAM) {
            if (length(rm.idx2) == 0){
              cn.lm1 = try(lm((norm.D$Mean)~gc), silent=T)
            } else {
              cn.lm1 = try(lm((norm.D$Mean)~gc, subset=-rm.idx2), silent=T)
            }
	} else {
            if (length(rm.idx2) == 0){
              cn.lm1 = try(lm((norm.D$Mean-norm.D$GMean)~gc))
            } else {
	...
	if (!singleBAM) {
            if (length(rm.idx2) == 0){
                cn.lm2 = try(lm((norm.D$Log)~gc), silent=T)
            } else {
