...
if [[ "$ASSEMBLY" == "hg38" ]]; then
    GOOD_BAD="$BASE_DIR/vcf2cna_prep/good.bad.new.hg38"
    WINDOW="$BASE_DIR/vcf2cna_prep/hg38_winbin_100bp.txt"
    CHR_SIZES="$BASE_DIR/vcf2cna_prep/chr_sizes_hg38.txt"
else
...
if [ "$DIPLOID_CHR" == "-1" ] && [ "$START_CHR" == "-1" ]  && [ "$END_CHR" == "-1" ]; then
    arguments="base_dir=\"$BASE_DIR\" SAMPLE=\"$FILENAME\" forced=T assembly=\"$ASSEMBLY\" working_directory=\"$WORK_DIR\"";
fi

if [ "$DIPLOID_CHR" -ne -1 ] && [ "$START_CHR" == "-1" ] && [ "$END_CHR" == "-1" ]; then
    arguments="base_dir=\"$BASE_DIR\" SAMPLE=\"$FILENAME\" forced=T assembly=\"$ASSEMBLY\" norm.chr=$DIPLOID_CHR working_directory=\"$WORK_DIR\"";
fi

if [ "$DIPLOID_CHR" -ne -1 ] && [ "$START_CHR" -ne -1 ] && [ "$END_CHR" -ne -1 ]; then
    arguments="base_dir=\"$BASE_DIR\" SAMPLE=\"$FILENAME\" forced=T assembly=\"$ASSEMBLY\" hg18=F norm.seg=c($DIPLOID_CHR,$START_CHR,$END_CHR) working_directory=\"$WORK_DIR\"";
fi
